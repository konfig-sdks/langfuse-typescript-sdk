...
import { readableStreamToString, LangfuseError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new LangfuseError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new LangfuseError(e, e.response?.data)
                }
                if (innerError instanceof LangfuseError) {
                    // Got "LangfuseError" from the above try block
                    throw innerError;
                }
